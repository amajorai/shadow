...
pub fn search_behavioral_context(
    ...
    let intervals: Vec<(u64, u64, String)> = stmt
        .query_map(
            params![format!("%{}%", target_app), start_ts as i64],
            |row| {
                let s: i64 = row.get(0)?;
                let e: i64 = row.get(1)?;
    ...
    for (interval_start, interval_end, app_name) in &intervals {
        let actions: Vec<(BehavioralAction, Option<String>)> = event_stmt
            .query_map(
                params![*interval_start as i64, *interval_end as i64],
                |row| {
...
fn relevance_score(seq: &BehavioralSequence, query_words: &[String]) -> f64 {
    ...
    let enriched_count = seq.actions.iter().filter(|a| a.ax_role.is_some()).count();
    if enriched_count > 0 {
        score += 0.5;
    }
...
pub fn count_enriched_clicks(conn: &Connection, start_us: u64, end_us: u64) -> Result<u64, String> {
    let count: i64 = conn
        .query_row(
...
mod tests {
    ...
    fn test_count_enriched_clicks() {
        ...
        let count = count_enriched_clicks(&conn, now - 1_000_000, now + 1_000_000).unwrap();
        assert_eq!(count, 1);
    }
