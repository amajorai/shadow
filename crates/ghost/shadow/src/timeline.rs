...
impl TimelineIndex {
    pub fn new(paths: &DataPaths) -> Result<Self, rusqlite::Error> {
        ...
        for &(col, col_type) in &[
            ...
            let sql = format!("ALTER TABLE events_index ADD COLUMN {} {}", col, col_type);
        ...
        for &(col, col_type) in &[
            ...
            let sql = format!("ALTER TABLE video_segments ADD COLUMN {} {}", col, col_type);
            let _ = conn.execute_batch(&sql);
        }
        ...
        for &(col, col_type) in &[
            ...
            let sql = format!("ALTER TABLE audio_segments ADD COLUMN {} {}", col, col_type);
            let _ = conn.execute_batch(&sql);
        }
    ...
    pub fn insert_batch(
        ...
        let tx = self.conn.transaction()?;
        {
            ...
            let mut close_stmt =
                tx.prepare("UPDATE app_focus_intervals SET end_ts = ?1 WHERE end_ts = 0")?;
    ...
    pub fn resolve_event_segment(&mut self, path: &str, ts: u64) -> Result<i64, rusqlite::Error> {
    ...
    pub fn find_app_at_timestamp(&self, ts_us: u64) -> Result<Option<AppContext>, rusqlite::Error> {
        ...
        match result {
            Ok((app_name, bundle_id)) => Ok(Some(AppContext {
                app_name,
                bundle_id,
            })),
    ...
    pub fn check_segment_integrity(&self) -> Result<IntegrityReport, rusqlite::Error> {
        ...
        let video_segments_count: i64 =
            self.conn
                .query_row("SELECT COUNT(*) FROM video_segments", [], |row| row.get(0))?;

        let audio_segments_count: i64 =
            self.conn
        ...
        let mut path_stmt = self
            .conn
            .prepare("SELECT path, status FROM segments WHERE status != 'open'")?;
    ...
    /// Keyframes captured within [start_ts_us, end_ts_us], ordered oldest→newest.
    ///
    /// `display_id = Some(id)` restricts to one display; `None` spans all displays.
    /// Powers the ambient "attach last N minutes" bundle: keyframes (~every 10s)
    /// are the reliable source, independent of whether any MP4 segment exists.
    pub fn keyframes_between(
        &self,
        display_id: Option<i64>,
        start_ts_us: u64,
        end_ts_us: u64,
    ) -> Result<Vec<crate::retention::KeyframeRecord>, rusqlite::Error> {
        ...
        match display_id {
            Some(d) => {
                let mut stmt = self.conn.prepare(
                    "SELECT display_id, ts, file_path, size_bytes FROM keyframes
                     WHERE display_id = ?1 AND ts >= ?2 AND ts <= ?3
                     ORDER BY ts ASC",
                )?;
    ...
    /// Delete keyframe rows older than `cutoff_ts` (Unix microseconds) and return
    /// the file paths that were removed so the caller can delete the JPEGs from
    /// disk. Used by the direct-keyframe retention sweep — frames captured as
    /// JPEGs without a backing MP4 segment are not covered by the segment-keyed
    /// warm→cold sweep, so this prunes them by age instead.
    pub fn prune_keyframes_before(&self, cutoff_ts: u64) -> Result<Vec<String>, rusqlite::Error> {
        let cutoff = cutoff_ts as i64;
        let mut stmt = self
            .conn
            .prepare("SELECT file_path FROM keyframes WHERE ts < ?1")?;
        let paths: Vec<String> = stmt
        ...
        drop(stmt);
        self.conn
            .execute("DELETE FROM keyframes WHERE ts < ?1", params![cutoff])?;
        Ok(paths)
    }
    ...
    pub fn query_ax_snapshots(
        ...
        if let bundle_id = app_bundle_id {
            ...
            let rows = stmt.query_map(params![start_ts as i64, end_ts as i64, limit], |row| {
                Ok(AxSnapshotRecord {
                    id: row.get(0)?,
                    timestamp_us: row.get::<_, i64>(1)? as u64,
                    app_bundle_id: row.get(2)?,
    ...
    pub fn delete_semantic_knowledge(&self, id: &str) -> Result<(), rusqlite::Error> {
        self.conn
            .execute("DELETE FROM semantic_knowledge WHERE id = ?1", params![id])?;
        Ok(())
    }
    ...
    pub fn record_directive_trigger(&self, id: &str, now_us: u64) -> Result<(), rusqlite::Error> {
        self.conn.execute(
            "UPDATE directives
...
mod tests {
    ...
    fn test_session_lifecycle() {
        ...
        index.finalize_session("test-session-1", 2000000).unwrap();
    }
    ...
    fn test_video_segment_lookup_range() {
        ...
        assert!(
            seg.is_none(),
            "Gap query should return None, not stale segment"
        );
    ...
    fn test_batch_insert() {
        ...
        let h2 = EventHeader {
            ...
            url: None,
            v: Some(2),
            ts_wall_us: Some(2000000),
            ts_mono_ns: Some(200),
            seq: Some(2),
            session_id: Some("s1".into()),
            source: Some("window_tracker".into()),
            display_id: None,
            pid: None,
            bundle_id: None,
            ax_role: None,
    ...
    fn test_seal_event_segment_with_compression() {
        ...
        index
            .seal_current_event_segment(2000000, Some("/path/to/hour14.msgpack.zst"), Some("zstd"))
            .unwrap();
    ...
    fn test_video_segment_mirrors_to_segments() {
        ...
        index
            .insert_video_segment(1, 1000, "/video/seg1.mp4")
            .unwrap();
        ...
        index
            .finalize_video_segment("/video/seg1.mp4", 2000)
            .unwrap();
    ...
    fn test_query_resolves_segment_path_through_segments_table() {
        ...
        index
            .insert(&header, "/path/to/hour14.msgpack", Some(seg_id))
            .unwrap();
    ...
    fn test_integrity_check() {
        ...
        let seg_id = index
            .resolve_event_segment("/path/to/seg.msgpack", 1000)
            .unwrap();
        let header = EventHeader {
            ts: 1000,
            track: 2,
        ...
        index
            .insert(&header, "/path/to/seg.msgpack", Some(seg_id))
            .unwrap();
    ...
    fn test_list_audio_segments_checkpoint_overlap() {
        ...
        let segs = index
            .list_audio_segments_after_checkpoint(id_mic, 1)
            .unwrap();
        assert_eq!(segs.len(), 1);
        ...
        let segs = index
            .list_audio_segments_after_checkpoint(id_sys, 10)
            .unwrap();
        assert_eq!(segs.len(), 0);
    }
    ...
    fn test_visual_change_log() {
        ...
        // Empty range
        let results = index
            .query_visual_changes(1, 5000000, 6000000, 0.3)
            .unwrap();
    ...
    fn test_checkpoint_no_advance_retries_same_segment() {
        ...
        let batch3 = index.list_audio_segments_after_checkpoint(id1, 10).unwrap();
        ...
        let batch4 = index.list_audio_segments_after_checkpoint(id2, 10).unwrap();
    ...
    fn test_register_keyframes_does_not_set_tier() {
        ...
        let hot_segs = index
            .list_video_segments_for_retention("hot", 5_000_000_000)
            .unwrap();
        assert!(hot_segs.is_empty(), "Segment should no longer be hot");
        let warm_segs = index
            .list_video_segments_for_retention("warm", 5_000_000_000)
            .unwrap();
        assert_eq!(warm_segs.len(), 1, "Segment should now be warm");
    }
    ...
    fn test_semantic_knowledge_upsert_and_query() {
        ...
        let results = index
            .query_semantic_knowledge(Some("preference"), 10)
            .unwrap();
        assert_eq!(results.len(), 1);
    ...
    fn test_semantic_knowledge_query_limit() {
        ...
        let results = index
            .query_semantic_knowledge(Some("preference"), 3)
            .unwrap();
    ...
    fn test_directive_no_expiry() {
        ...
        index
            .upsert_directive("dir-1", "habit", "any", "action", 3, 1000000, None, "test")
            .unwrap();
    ...
    fn test_directive_record_trigger() {
        ...
        index
            .upsert_directive(
                "dir-1", "reminder", "trigger", "action", 5, 1000000, None, "test",
            )
            .unwrap();
    ...
    fn test_directive_priority_ordering() {
        ...
        index
            .upsert_directive(
                "dir-high", "reminder", "t2", "high", 10, 2000000, None, "test",
            )
            .unwrap();
        index
