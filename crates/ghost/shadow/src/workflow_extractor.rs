...
fn segment_actions(actions: &[RawAction]) -> Vec<ActionSegment> {
    ...
    for action in actions {
        ...
        let should_split = if current_actions.is_empty() {
            ...
            let time_gap =
                action.ts.saturating_sub(current_actions.last().unwrap().ts) > SEGMENT_GAP_US;
...
fn find_recurring_patterns(segments: &[ActionSegment]) -> Vec<RecurringPattern> {
    ...
    for seg in segments {
        by_app.entry(seg.app_name.clone()).or_default().push(seg);
    }
...
fn build_workflow(pattern: RecurringPattern) -> ExtractedWorkflow {
    ...
    let best_exemplar = pattern.exemplars.iter().max_by_key(|e| e.end_ts).unwrap();
    ...
    let name = if let Some(step) = distinctive_step {
        let action_desc = step.target_title.as_deref().unwrap_or(&step.action_type);
