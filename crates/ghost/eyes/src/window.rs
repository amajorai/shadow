        ...
        if let Ok(auto2) = walker_result {
            if let Ok(walker) = auto2.ControlViewWalker() {
                if let Ok(child) = walker.GetFirstChildElement(element) {
                    ...
                    loop {
                        if let Some(url) =
                            find_url_in_element(automation, &current, names, depth + 1)
                        {
                            return Some(url);
                        }
...
fn macos_frontmost_window() -> Option<WindowInfo> {
    ...
    unsafe fn ax_str(el: *const c_void, attr: &[u8]) -> Option<String> {
        ...
        let err = AXUIElementCopyAttributeValue(el, cf, &mut val);
        CFRelease(cf);
    ...
    unsafe {
        ...
        let title = if !ax_app.is_null() {
            ...
            let err = AXUIElementCopyAttributeValue(ax_app, win_cf, &mut win);
            CFRelease(win_cf);
            let t = if err == 0 && !win.is_null() {
                let s = ax_str(win, b"AXTitle\0");
                CFRelease(win);
                s
            } else {
                None
...
fn linux_active_window() -> Option<WindowInfo> {
    ...
    let title = {
        let p = conn
            ...
            .reply()
            .ok()?;
        String::from_utf8_lossy(&p.value).to_string()
    };
    let pid = conn
    ...
    let app_name = {
        ...
        let inst = parts.next().unwrap_or("").to_string();
        parts.next().unwrap_or(&inst).to_string()
    };
    Some(WindowInfo {
        title,
        app_name,
...
mod tests {
    ...
    #[test]
    fn window_info_serde_roundtrip() {
        let w = WindowInfo {
            title: "Untitled — Editor".to_string(),
        ...
        assert_eq!(back.title, "Untitled — Editor");
        assert_eq!(back.app_name, "Code");
        assert_eq!(back.bundle_id.as_deref(), Some("com.microsoft.VSCode"));
        assert_eq!(back.pid, 4321);
        assert!(back.url.is_none());
    }

    #[test]
    fn window_info_url_field_serializes() {
        let w = WindowInfo {
        ...
        assert!(v["bundle_id"].is_null());
    }

    #[test]
    fn app_info_serde_roundtrip() {
        let a = AppInfo {
        ...
        assert_eq!(back.name, "Finder");
        assert_eq!(back.bundle_id.as_deref(), Some("com.apple.finder"));
    ...
    #[cfg(target_os = "macos")]
    #[test]
    fn macos_window_tracker_new_ok() {
        assert!(MacOSWindowTracker::new().is_ok());
    }

    #[cfg(target_os = "macos")]
    #[test]
    fn macos_active_window_is_well_formed_when_present() {
        // Reads NSWorkspace.frontmostApplication + a bounded AXFocusedWindow query
        // (no prompt when Accessibility is denied). None is valid in a headless
        // session; when Some, the reported shape must be internally consistent.
        let tracker = MacOSWindowTracker::new().unwrap();
        let win = rt().block_on(async { tracker.get_active_window().await });
        if let Some(w) = win {
            assert!(w.pid >= 0, "pid must be non-negative");
            // app_name is derived from localizedName; bundle_id/url are optional.
            let _ = (&w.title, &w.app_name, &w.bundle_id, &w.url);
        }
    }

    #[cfg(target_os = "macos")]
    #[test]
    fn macos_active_app_mirrors_window() {
        let tracker = MacOSWindowTracker::new().unwrap();
        let app = rt().block_on(async { tracker.get_active_app().await });
        if let Some(a) = app {
            assert!(a.pid >= 0);
            // get_active_app always reports the frontmost app as focused.
            assert!(a.is_focused);
        }
    }
}
