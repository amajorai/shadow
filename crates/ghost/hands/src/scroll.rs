    ...
    let (flags, wheel_delta) = match direction {
        ...
        _ => {
            return Err(anyhow::anyhow!(
                "Unknown scroll direction '{}'; use up, down, left, or right",
                direction
            ))
        }
    };
...
mod tests {
    ...
    fn scroll_rejects_unknown_direction() {
        // An invalid direction errors out at the match arm — before any scroll event
        // is posted. The four valid directions are not exercised here precisely
        // because they would inject real wheel input into the OS.
        let err = scroll(0, 0, "diagonal", 1).unwrap_err();
        assert!(
            err.to_string().contains("Unknown scroll direction"),
            "got: {err}"
        );
    }
}
